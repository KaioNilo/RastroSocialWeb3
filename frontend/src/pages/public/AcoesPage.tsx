        ...
        <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
          <div className="flex space-x-4">
            <button
              ...
              className={`px-4 py-2 rounded-lg transition ${filtro === 'recente' ? 'bg-[#2322E3] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            >
              Mais Recente
            </button>
            <button
              onClick={() => setFiltro('antigo')}
              className={`px-4 py-2 rounded-lg transition ${filtro === 'antigo' ? 'bg-[#2322E3] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            >
              Mais Antigo
            </button>
            <button
              onClick={() => setFiltro('pontos')}
              className={`px-4 py-2 rounded-lg transition ${filtro === 'pontos' ? 'bg-[#2322E3] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            >
              Mais Pontos
            </button>
          </div>
          <div className="text-sm text-gray-500">
            {carregando ? 'Carregando...' : `${eventosOrdenados.length} ação${eventosOrdenados.length !== 1 ? 'es' : ''} encontrada${eventosOrdenados.length !== 1 ? 's' : ''}`}
          </div>
        </div>
